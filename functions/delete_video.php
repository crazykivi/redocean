...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare("SELECT authorsID FROM video WHERE urlVideo = ?");
        $stmt->execute([$urlVideo]);
        $author = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($author && $author['authorsID'] == $currentUserId) {
            ...
            echo json_encode(['success' => false, 'error' => 'У вас нет прав для удаления этого видео.']);
        }
    } else {
